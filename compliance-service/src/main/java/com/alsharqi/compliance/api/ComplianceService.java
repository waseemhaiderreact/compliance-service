...
import com.alsharqi.compliance.audittrail.AuditTrailRepository;
...
import com.alsharqi.compliance.compliancetemplate.ComplianceTemplateRepository;
import com.alsharqi.compliance.contact.Contact;
import com.alsharqi.compliance.contact.ContactRepository;
import com.alsharqi.compliance.document.DocumentRepository;
import com.alsharqi.compliance.document.Document;
...
public class ComplianceService {
    ...
    ResponseEntity<?> saveCompliance(Compliance compliance){
        ...
        try{
            LOGGER.info("saving a compliance ");
    ...
    ResponseEntity<?> updateCompliance(Compliance compliance){
        ...
        try{
            ...
            responseEntity = new ResponseEntity<>("Cannot update a compliance", HttpStatus.INTERNAL_SERVER_ERROR);
        }
        return responseEntity;
    }
...
//    }
    ...
    //getting a compliance template based on compliance type
    ResponseEntity<?> getComplianceTemplateByTypeOfCompliance(String typeOfCompliance){
        ResponseEntity responseEntity = null;
        try{
            LOGGER.info("getting a compliance template by its Type ",typeOfCompliance);
            responseEntity = new ResponseEntity<>
                    (complianceTemplateRepository.findComplianceTemplateByTypeOfCompliance(typeOfCompliance), HttpStatus.OK);
        }catch (Exception e){
            LOGGER.info("cannot get a compliance template by its type",e);
            responseEntity = new ResponseEntity<>("Cannot get a compliance template by its type", HttpStatus.INTERNAL_SERVER_ERROR);
        }
        return responseEntity;
    }
    ...
    @Transactional
    public List<ComplianceTemplate> getAllComplianceTemplates(){
        return complianceTemplateRepository.findAll();
    }

    //getting all the audit trails based on compliance number
    @Transactional
    public List<AuditTrail> getAuditByComplianceNumber(String complianceNumber){
       return  auditTrailRepository.getAuditTrailByComplianceNumber(complianceNumber);
    }
    ...
    ResponseEntity<?> saveComplianceMapper(Compliance request, List<MultipartFile> complianceDocument,String username){
        ...
        try{
            ...
            if(compliance!=null){
                ...
                if(list.size()>0)
                {
                    for(MultipleFileUploadPOSTResponse mapper: list)
                    {
                        Document document = new Document();
                        document.setDocumentName(mapper.getDocumentName());
    ...
    @Async
    List<MultipleFileUploadPOSTResponse> uploadMultipleCompliancesDocuments(List<MultipartFile> complianceDocument,Long id,String username){
        ResponseEntity<MultipleFileUploadPOSTResponse[]> responseEntity=null;
        List<MultipleFileUploadPOSTResponse> list=new ArrayList<>();
        try {
            ...
            LOGGER.error("Failed to send Documents to Document Service",e);
        }
        return list;
    }

    /** Helper Functions for the file conversion **/
    public static File convert(MultipartFile file)
    {
        ...
        {
            LOGGER.error("Failed to convert Document to fileOutput Stream ",e);
        }
        return convFile;
    }

    //getting relative date for front end
    public static Date getRelativeTimeUsingDate() {
        LocalDate localDate = LocalDate.now();
        String dt = localDate.toString();
        String parsedDate =  LocalDate.parse(dt).plusDays(2).toString();
        localDate = LocalDate.parse(parsedDate);
        Date date = java.sql.Date.valueOf(localDate);
        return date;
    }

    //saving a compliance along with the document
//    ResponseEntity<?> addLegalEntityCustomer(Compliance request, Long parentOrgId, List<MultipartFile> company,List<MultipartFile> manager,String username)throws ApplicationException
//    {
//        LOGGER.info("Adding Compliance : "+parentOrgId);
////        Set<Contact> contacts;
////        Set<Location> locations;
////        boolean locationFlag;
////        boolean contactsFlag;
//        Compliance organization=new Compliance();
//        try {
//            if(company.size() == 0 && manager.size() == 0)
//            {
//                return new ResponseEntity<>(new DefaultResponse("Failure","Failed to add compliance, no documents were attached","F01"), HttpStatus.CONFLICT);
//            }
//            List<MultipleFileUploadPOSTResponse> list=uploadMultipleCustomersDocuments(company,manager,organization.getId(),username);
//            if (list.size() > 0)
//            {
//                for(MultipleFileUploadPOSTResponse mapper: list)
//                {
...
//                    document.setCustomerType(mapper.getCustomerType());
//                    documentRepository.save(document);
//                }
//                return new ResponseEntity<>(new DefaultResponse("Success","Successfully added legal entity","S01"), HttpStatus.OK);
//            }
//            else
//            {
//                return new ResponseEntity<>(new DefaultResponse("Failure","Fail to add legal entity due to documents","S01"), HttpStatus.OK);
//            }
//            return new ResponseEntity<>(new DefaultResponse("Failure","Fail to add legal entity no contact or location added","F01"), HttpStatus.CONFLICT);
//        }
//        catch (Exception e){
//            LOGGER.error("Failed to add legal entity",e);
//            throw new ApplicationException("F102",e.getMessage());
//        }
//    }
